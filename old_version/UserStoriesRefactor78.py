...
def get_date(info_dict,info_id,tag):
    """
    extracts the date from the dictionary passed for the id passed for a given tag
    """
    try:
        date = info_dict.get(info_id).get(tag)
        return date
    except:
        return 'NA'    
...
def dates_before(date1, date2):
    """
    returns true if date1 occurs before date2 with the buffer added
    """
    d1 = date1[0].split(' ')[-1]
    d2 = date2[0].split(' ')[-1]
...
class TestGetDate(unittest.TestCase):
    """
    This class tests if the date retured is same as the expected date
    """
    ...
    def testGetDate(self):
        ...
        d1 = get_date(individuals,'I6','DEAT')
        d2 = get_date(family,'F2','MARR')
        check1 = dates_before(d1,d2 )
        self.assertFalse(check1)
        
        d3 =  get_date(family,'F7','MARR')
        d4 =  get_date(family,'F7','DIV')
        check2 = dates_before(d3,d4)
        self.assertTrue(check2)
        
        d5 = get_date(individuals,'I8','BIRT')
        d6 = get_date(individuals,'I8','DEAT')
        check3 = dates_before(d5,d6 )
        self.assertTrue(check3)
        
        d7 = get_date(individuals,'I4','DEAT')
        d8 = get_date(individuals,'I4','BIRT')
        check4 = dates_before(d7,d8)
...
class TestDateBefore(unittest.TestCase):
    """
    This class tests if the first date occurs before the second date
    """
    ...
    def testDateBefore(self):
        ...
        condition_Satisfied_1 = get_date(individuals,'I6','DEAT')
        self.assertEqual(condition_Satisfied_1, ['12 FEB 0283'])

        condition_Satisfied_2 = get_date(family,'F2','MARR')
        self.assertEqual(condition_Satisfied_2, ['8 MAY 0282'])

        
        condition_Satisfied_3 =  get_date(family,'F3','MARR')
        self.assertEqual(condition_Satisfied_3, ['7 JAN 0280'])

        condition_Satisfied_4 =  get_date(family,'F7','DIV')
        self.assertEqual(condition_Satisfied_4, ['26 MAY 0301'])

        condition_Satisfied_5 = get_date(individuals,'I7','BIRT')
        self.assertEqual(condition_Satisfied_5, ['11 OCT 0283'])
...
class TestBirthBeforeMarriage(unittest.TestCase):
    ...
    Birth before marriage specific test case
    """
